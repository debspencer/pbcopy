...
func (pb *PbReader) NextField(startIdx int) (*PbField, int, error) {
	...
	for idx := startIdx; idx < maxIdx; idx++ {
		...
		return pb.makeField(memberName, memberType), idx, nil
	}
	return nil, -1, nil
}
...
func (pb *PbWriter) writeHeader(sb *strings.Builder) {
	sb.WriteString("// AUTOMAGICALLY GENERATED by protomkcp --- DO NOT EDIT\n\n")
...
func (pb *PbReader) makeField(memberName string, memberType string) *PbField {
	f := &PbField{
		memberName: memberName,
		filterNull: pb.filterNull,
	}
...
func (pb *PbField) writeMap() {
	...
	if pb.isDeepCopy() {
		...
		if pb.filterNull {
			...
			pb.add("out." + pb.memberName + "[k] = nil")
		}
		pb.add("} else {")
		defer pb.add("}")
	}
...
func (pb *PbField) writeSliceData() {
	...
	if pb.isDeepCopy() {
		...
		if pb.filterNull {
			...
			pb.add("out." + pb.memberName + "[i] = nil")
		}
		pb.add("} else {")
		defer pb.add("}")
	}
...
func (pb *PbField) isDeepCopy() bool {
